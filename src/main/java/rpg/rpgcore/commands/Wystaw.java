...
public class Wystaw implements CommandExecutor {
    ...
    public boolean onCommand(final CommandSender sender, final Command cmd, final String label, final String[] args) {
        ...
        if (args.length == 1) {
            ...
            try {
                ...
                this.wystawGUI(player, player.getItemInHand().clone(), cena);
    ...
    private void wystawGUI(final Player player, final ItemStack is, final double kwota) {
        ...
        lore.add(" ");
        lore.add("&2Cena: &6" + Utils.kasaFormat.format(kwota) + " &2$");
        lore.add(" ");
        lore.add("&8Wystawiajac ten przedmiot za kwote &6&o" + Utils.kasaFormat.format(kwota) + " &2$");
        lore.add("&8Zaplacisz podatek za wystawienie przedmiotu");
        lore.add("&8W wysokosci &c5% ceny wystawienia&8, czyli &6&o" + Utils.kasaFormat.format((kwota * 5) /100) + " &2$" );
        lore.add(" ");
