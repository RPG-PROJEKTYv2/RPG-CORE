...
public class MisjeCommand extends CommandAPI {
    ...
    public void openMissionGUI(Player player, Player target) {
        ...
        final User user = rpgcore.getUserManager().find(target.getUniqueId());
        if (user == null) {
            player.sendMessage(Utils.format(Utils.SERVERNAME + "&cCos poszlo nie tak i nie udalo sie stworzyc GUI aktualnych misji gracza &6" + target.getName() + "&c!"));
            return;
        }
        // PIERWSZY RZAD
        gui.setItem(0, new ItemBuilder(Material.HOPPER).setName("&c&lWyslannik").toItemStack());

        final WyslannikUser wyslannikUser = rpgcore.getWyslannikNPC().find(target.getUniqueId()).getWyslannikUser();
        ...
        final LowcaUser lowcaUser = rpgcore.getLowcaNPC().find(target.getUniqueId()).getLowcaUser();
        ...
        gui.setItem(11, rpgcore.getPrzyrodnikNPC().getOddajItemyItem(rpgcore.getPrzyrodnikNPC().find(target.getUniqueId()).getUser()));
        ...
        final KolekcjonerUser kolekcjonerUser = rpgcore.getKolekcjonerNPC().find(target.getUniqueId()).getKolekcjonerUser();
        ...
        gui.setItem(30, rpgcore.getLesnikNPC().getCurrentMissionItem(rpgcore.getLesnikNPC().find(target.getUniqueId()).getUser()));
        ...
        if (user.getLvl() < 60) {
            ...
            final GornikUser gornikUser = rpgcore.getGornikNPC().find(target.getUniqueId()).getGornikUser();
        ...
        final MetinologUser metinologUser = rpgcore.getMetinologNPC().find(target.getUniqueId()).getMetinologUser();
        ...
        if (user.getLvl() < 30) {
            ...
            final RybakUser rybakUser = rpgcore.getRybakNPC().find(target.getUniqueId()).getRybakUser();
        ...
        final MagazynierUser magazynierUser = rpgcore.getMagazynierNPC().find(target.getUniqueId());
