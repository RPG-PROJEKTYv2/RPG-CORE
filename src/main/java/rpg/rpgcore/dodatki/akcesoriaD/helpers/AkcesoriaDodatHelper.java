...
public class AkcesoriaDodatHelper {
    ...
    public static ItemStack createPas(final int defLudzie, final int defMoby, final int lvl, final String... name) {
        return new ItemBuilder(Material.LEASH).setName(Arrays.stream(name).map(arg -> arg + " ").collect((Collector) Collectors.joining()).toString().trim()).setLore(Arrays.asList(
                "&7Typ: &cAkcesorium Dodatkowe",
    ...
    public static ItemStack createMedalion(final int srdmg, final int dodatkoweHp, final int lvl, final String... name) {
        return new ItemBuilder(Material.FIREBALL).setName(Arrays.stream(name).map(arg -> arg + " ").collect((Collector) Collectors.joining()).toString().trim()).setLore(Arrays.asList(
                "&7Typ: &cAkcesorium Dodatkowe",
    ...
    public static ItemStack createEnergia(final int mDmg, final int def, final int blok, final double przebicie, final int mspeed, final int lvl, final String... name) {
        return new ItemBuilder(Material.MINECART).setName(Arrays.stream(name).map(arg -> arg + " ").collect((Collector) Collectors.joining()).toString().trim()).setLore(Arrays.asList(
                "&7Typ: &cAkcesorium Dodatkowe",
