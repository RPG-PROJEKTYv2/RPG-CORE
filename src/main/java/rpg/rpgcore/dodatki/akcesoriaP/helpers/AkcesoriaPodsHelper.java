...
public class AkcesoriaPodsHelper {
    public static ItemStack createTarcza(final double def, final double blok, final int hp, final int lvl, final String... name) {
        return new ItemBuilder(Material.ITEM_FRAME).setName(Arrays.stream(name).map(arg -> arg + " ").collect((Collector) Collectors.joining()).toString()).setLore(Utils.format(Arrays.asList(
    ...
    public static ItemStack createKolczyki(final double ludzie, final double odpo, final int mspeed, final int lvl, final String... name) {
        return new ItemBuilder(Material.HOPPER_MINECART).setName(Arrays.stream(name).map(arg -> arg + " ").collect((Collector) Collectors.joining()).toString()).setLore(Utils.format(Arrays.asList(
                "&7Silny przeciwko ludziom: &c" + ludzie + "%",
                "&7Odpornosc na Ludzi: &c" + odpo + "%",
                "&7Zmniejszona Szybkość Ruchu: &c-" + mspeed,
                " ",
                "&cWymagany poziom: &6" + lvl
                )
    ...
    public static ItemStack createNaszyjnik(final int ddmg, final double kryt, final double srdmg, final int lvl, final String... name) {
        return new ItemBuilder(Material.STORAGE_MINECART).setName(Arrays.stream(name).map(arg -> arg + " ").collect((Collector) Collectors.joining()).toString()).setLore(Utils.format(Arrays.asList(
    ...
    public static ItemStack createPierscien(final double przebicie, final double kryt, final int speed, final int lvl, final String... name) {
        return new ItemBuilder(Material.EXPLOSIVE_MINECART).setName(Arrays.stream(name).map(arg -> arg + " ").collect((Collector) Collectors.joining()).toString()).setLore(Utils.format(Arrays.asList(
                "&7Szansa na Przebicie Pancerza: &c" + przebicie + "%",
                "&7Wzmocnienie Ciosu Krytycznego: &c" + kryt + "%",
                "&7Zwiekszona Szybkosc Ruchu: &c+" + speed,
                " ",
                "&cWymagany poziom: &6" + lvl
                )
        )).addTagDouble("przebicie", przebicie).addTagDouble("wkryt", kryt).addTagInt("speed", speed).addTagInt("lvl", lvl).hideFlag().toItemStack().clone();
    }
    ...
    public static ItemStack createDiadem(final double srdmg, final double potwory, final double exp, final int lvl, final String... name) {
        return new ItemBuilder(Material.WATCH).setName(Arrays.stream(name).map(arg -> arg + " ").collect((Collector) Collectors.joining()).toString()).setLore(Utils.format(Arrays.asList(
