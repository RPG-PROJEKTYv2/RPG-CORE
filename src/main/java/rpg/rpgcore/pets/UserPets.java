...
public class UserPets {
    ...
    public void removePet(final ItemStack pet) {
        ...
        final List<ItemStack> newPety = new ArrayList<>();
        for (final ItemStack is : this.pety) {
            newPety.add(is);
        }
        newPety.remove(pet);
        this.pety = newPety;
    ...
    public void addPet(final ItemStack pet) {
        final List<ItemStack> newPets = new ArrayList<>();
        for (final ItemStack pet1 : this.pety) {
            newPets.add(pet1);
        }
        newPets.add(pet);
        this.pety = newPets;
