...
public class AkcesoriaHelper {
    ...
    public static ItemStack createMedalion(final String name, final int przeszywka, final int dmg, final int wymaganyPoziom) {
        return new ItemBuilder(Material.FIREBALL).setName(name.replaceAll("_", " ")).setLore(Arrays.asList("&3Przeszycie Bloku: &f" + przeszywka + "%", "&3Obrazenia: &f" + dmg, "&c&oWymagany Poziom: &6" + wymaganyPoziom)).toItemStack().clone();
    }

    public static ItemStack createPas(final String name, final int krytyk, final int sredniDmg, final int wymaganyPoziom) {
        return new ItemBuilder(Material.LEASH).setName(name.replaceAll("_", " ")).setLore(Arrays.asList("&3Szansa na Krytyk: &f" + krytyk + "%", "&3Srednie Obrazenia: &f" + sredniDmg + "%", "&c&oWymagany Poziom: &6" + wymaganyPoziom)).toItemStack().clone();
    }

    public static ItemStack createKolczyki(final String name, final int hp, final int silnyNaLudzi, final int wymaganyPoziom) {
        return new ItemBuilder(Material.HOPPER_MINECART).setName(name.replaceAll("_", " ")).setLore(Arrays.asList("&3Dodatkowe HP: &f" + hp, "&3Silny na Ludzi: &f" + silnyNaLudzi + "%", "&c&oWymagany Poziom: &6" + wymaganyPoziom)).toItemStack().clone();
    }

    public static ItemStack createSygnet(final String name, final int hp, final int silnynamoby, final int wymaganyPoziom) {
        return new ItemBuilder(Material.GOLD_NUGGET).setName(name.replaceAll("_", " ")).setLore(Arrays.asList("&3Dodatkowe HP: &f" + hp, "&3Silny przeciwko Potworom: &f" + silnynamoby + "%",  "&c&oWymagany Poziom: &6" + wymaganyPoziom)).toItemStack().clone();
    }
