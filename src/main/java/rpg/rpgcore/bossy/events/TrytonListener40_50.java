...
public class TrytonListener40_50 implements Listener {
    ...
    public void onInteract(final PlayerInteractEvent e) {
        ...
        if (e.getItem().getType() != Material.FIREBALL || !e.getItem().hasItemMeta() || !e.getItem().getItemMeta().hasDisplayName()
                || !Utils.removeColor(e.getItem().getItemMeta().getDisplayName()).equals("Przywolanie * Tryton")) return;

        final Player player = e.getPlayer();
        final User user = RPGCORE.getInstance().getUserManager().find(player.getUniqueId());

        if (!player.getWorld().getName().equals("40-50map")) {
        ...
        if (bossyManager.getBoss40_50count() == 3) {
            player.sendMessage(Utils.format("&8&l(&4&lBOSS&8&l) &8>> &cW tym momencie nie mozesz przywolac wiecej &e&lTrytonow! &7(Limit na mape: 3)"));
            return;
        }
        e.setUseInteractedBlock(Event.Result.DENY);
        e.setUseItemInHand(Event.Result.DENY);
        e.setCancelled(true);
        player.getInventory().removeItem(new ItemBuilder(Bossy.I40_50.getItemStack().clone()).setAmount(1).toItemStack());
