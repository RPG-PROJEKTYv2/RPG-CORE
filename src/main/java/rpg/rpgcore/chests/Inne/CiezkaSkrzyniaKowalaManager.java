...
public class CiezkaSkrzyniaKowalaManager {
    ...
    public CiezkaSkrzyniaKowalaManager() {
        ...
        this.kowal.add(new Items("5", 8.0, GlobalItem.getItem("I10", 2), 1));
        this.kowal.add(new Items("6", 9.0, GlobalItem.getItem("I_OCZYSZCZENIE", 1),1));
        this.kowal.add(new Items("8", 9.0, GlobalItem.getItem("I_FRAGMENT_STALI",3),1));
        this.kowal.add(new Items("7", 11.0, GlobalItem.getItem("I10", 1), 1));
        this.kowal.add(new Items("9", 13.5, GlobalItem.getItem("I_FRAGMENT_STALI",2),1));
    }
            ...
            if (item.getChance() >= 100.0 || item.getChance() > ThreadLocalRandom.current().nextDouble(0.0, 100.0)) {
                if (item.getRewardItem().getType() == Material.MINECART) {
                    if (item.getRewardItem().getItemMeta().getDisplayName().contains("Energia Piekielnego Kowala")) {
                        ...
                        Bukkit.getServer().broadcastMessage(" ");
                        player.getInventory().addItem(AkcesoriaDodatHelper.createEnergia(ChanceHelper.getRandInt(-33, -25), ChanceHelper.getRandInt(27, 61), ChanceHelper.getRandInt(25, 55), ChanceHelper.getRandDouble(0.1, 0.4), ChanceHelper.getRandInt(-50, -45), ChanceHelper.getRandInt(60, 90), "&4&lEnergia Piekielnego Kowala"));
                        return;
                    }
                    return;
                }
