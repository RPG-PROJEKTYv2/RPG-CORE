...
public class MessageCommand extends CommandAPI {
    ...
    public void executeCommand(CommandSender sender, String[] args) throws IOException {
        ...
        if (rpgcore.getUserManager().find(targetUUID).getRankUser().isHighStaff() && rpgcore.getUserManager().find(targetUUID).isAdminCodeLogin()
                && (!rpgcore.getUserManager().find(playerUUID).getRankUser().isStaff() || (rpgcore.getUserManager().find(playerUUID).getRankUser().isStaff() && !rpgcore.getUserManager().find(playerUUID).isAdminCodeLogin()))) {
